...
forwardback.ded <- function (x, Pi, delta, distn, pm, pn = NULL, fortran = FALSE, prob, v=0) {
...
read.cross.msg <- function(file.AA, file.AB, sex=rep("f", length(indivs)), phenos=NULL, na.rm=FALSE) {
...
## extracted from write-hmm-data.R
decode.pileup.bases.count <- function(x, ref, dir, allele.states, v=0) {
  ## x is a vector of pileup base codes. Examples are
  ## c..c   .+2cc   ^fa^f,^f,
  ## ref is a vector containing the corresponding reference alleles
  ## http://samtools.sourceforge.net/pileup.shtml
  ...
  if(v) cat("countalleles...")
  tmpfile=tempfile()
  cat(x, file=tmpfile, sep="\n")
  pipa <- pipe(sprintf("%s/countalleles < %s", dir, tmpfile))
  tab <- matrix(scan(pipa, what=integer(), quiet=TRUE), ncol=5, byrow=TRUE)
...
getContigLengths <- function(f="msg.chrLengths", v=0){
  contigs <- fread(f,header=F,data.table=F,sep='\t')[,1:2]
  names(contigs) = c('chr', 'length')
  rownames(contigs) <- contigs$chr
  contigs <- contigs[order(contigs$length),]
  if(v) cat(sprintf('%s contains %s contigs spanning %.2f Mb\n', f, nrow(contigs), sum(contigs$length)/1e6))
  return(contigs)
}

readProbs <- function(indiv, outdir, contigs=NULL, raw=F){
  ## convert probs to long df
  ## indiv, contig, pos, bad, par1ref, par2ref, count, est, haplotype, prob, hid (numeric haplotype id)
  probf = sprintf("%s/%s.hmmprob.txt", outdir, indiv)
  stopifnot(file.exists(probf))
  data <- fread(probf, data.table=F, showProgress = F, verbose = F, sep='\t')
  if(!is.null(contigs)) data <- subset(data, contig %in% contigs)
  if(raw) return(data)
  data$est = names(data)[22:24][data$est]
  ...
  md$prob[md$prob>1] <- 1
  md$prob[md$prob<0] <- 0
  ## inf values seen at rfac 1e-12. coerce to estimate
  if(sum(is.infinite(md$prob))>0) cat(sprintf("WARNING infinite values seen in probs for %s\n", indiv))
  md$prob[is.infinite(md$prob) & md$haplotype==md$est] <- 1
...
filterBreaks <- function(probs, minpp=0.5, minBlock=1e4, minBlockMarkers=5){
  # hard filter posterior probs and filter out blocks < minBlock bp supported by < minBlockMarkers
  # adds new ancestry column with filtered data as NA
  probs <- subset(probs, prob > minpp)
  probs <- probs[order(probs$contig, probs$pos),]
  o = do.call(rbind, lapply(split(probs, probs$contig), function(x) {
    rl <- rle(as.character(x$haplotype))
    rll = rl$lengths
    if(length(rll)>1){
      rl$values[rll < minBlockMarkers] <- NA
      cs = cumsum(rll)
      pox = x$pos
      rlbp <- c(pox[cs[1]]-pox[1], sapply(2:length(rll), function(i) pox[cs[i]]-pox[cs[i-1]]))
      rl$values[rlbp < minBlock] <- NA
    }
    nb = sum(!is.na(rl$values))-1; if(nb<0) nb=NA
    cbind(x, out = rep.int(rl$values, times=rl$lengths), nb)
  }))
  rownames(o) <- NULL
  o
}
...
infill <- function(x){
  # fill in interior NAs where flanking values agree
  apply(x, 2, function(y) {
    ...
    if(length(y)>2){
      ...
      ychar[is.na(ychar)] <- 'X'
      rl = rle(ychar)
      nax = which(rl$values=='X')
      if(len(nax)>0){
        if(nax[1]==1) nax = nax[-1]
        if(length(nax)>0) if(nax[length(nax)]==length(rl$values)) nax = nax[-length(nax)]
        if(length(nax)>0){
          ok = rl$values[nax-1] == rl$values[nax+1]
          if(is.na(ok[length(ok)])) ok = ok[-length(ok)]
          for(i in seq_along(nax[ok])){
            runi = nax[ok][i]
            il = sum(rl$lengths[1:(runi)-1])+1
            ir = sum(rl$lengths[1:runi])
            out[il:ir] <- out[il-1]
          }  
        }
      }  
...
argsplit <- function(arg, delim=','){
  unlist(strsplit(arg,split=delim))
}

sus <- function(x, nsd=3, ix=F) {
  # filter a vector based on mean+-sd, returning outlier values or logical vector
  i=(x>(mean(x, na.rm=T)+nsd*sd(x, na.rm=T)))|(x<(mean(x, na.rm=T)-nsd*sd(x, na.rm=T)))
  if(ix) return(i)
  x[i]
}
